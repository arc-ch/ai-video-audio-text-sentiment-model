...
# TextEncoder: Uses frozen BERT to get text features, projects them down to 128 dims.

# VideoEncoder: Uses frozen 3D ResNet (R3D-18) backbone, replaces final layer with trainable 128-dim projection.

# AudioEncoder: CNN layers for audio (frozen), followed by a trainable projection.

# MultimodalSentimentModel: Combines features from all three modalities, fuses them, and outputs emotion (7 classes) and sentiment (3 classes) predictions.

# compute_class_weights: Counts classes in dataset to create inverse frequency weights for balanced loss.

# MultimodalTrainer: Manages training, validation, logging to TensorBoard, optimizer scheduling, and metric calculation.
